  ...
  const handleDragEnd = (event) => {
    ...
    for (let wi = 0; wi < plan.weeks.length; wi++) {
      for (let di = 0; di < 7; di++) {
        ...
        if (workoutIds.includes(overId) || `day-${wi}-${di}` === overId) {
          destWeekIndex = wi;
          destDayIndex = di;
    ...
    const sourceWorkouts = [
      ...plan.weeks[sourceWeekIndex].days[sourceDayIndex].workouts,
    ];
    const destWorkouts = [
      ...plan.weeks[destWeekIndex].days[destDayIndex].workouts,
    ];
    const workoutToMove = sourceWorkouts[sourceWorkoutIndex];

    if (sourceWeekIndex === destWeekIndex && sourceDayIndex === destDayIndex) {
      // Reorder within the same day
      if (destWorkoutIndex === -1) {
        destWorkouts.push(workoutToMove);
      } else {
        destWorkouts.splice(destWorkoutIndex, 0, workoutToMove);
      }
      sourceWorkouts.splice(sourceWorkoutIndex, 1);
      if (destWorkoutIndex !== -1 && sourceWorkoutIndex < destWorkoutIndex) {
        destWorkouts.splice(sourceWorkoutIndex, 0, workoutToMove);
        destWorkouts.splice(destWorkoutIndex + 1, 1);
      }
    } else {
        ...
        if (wi === sourceWeekIndex) {
          return {
            ...
            days: w.days.map((d, di) =>
              di === sourceDayIndex ? { ...d, workouts: sourceWorkouts } : d
            ),
          };
        }
        if (wi === destWeekIndex) {
          return {
            ...w,
            days: w.days.map((d, di) =>
              di === destDayIndex ? { ...d, workouts: destWorkouts } : d
            ),
          };
        }
        return w;
    ...
    <DndContext collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
      <div className="w-full overflow-x-auto lg:overflow-x-hidden">
                ...
                >
                  ...
                  !collapsedWeeks.has(weekIndex) ? (
                    <>
                      ...
                      >
                        <div className="flex items-center self-center flex-grow">
                          {day.workouts.length > 0 ? (
                            ...
                            <span className="align-middle">Rest Day</span>
                          )}
                        </div>
                      </SortableContext>
        ...
        <div className="mt-4 flex justify-center">
          {/*           <button
            onClick={addWeek}
            className="inline-flex items-center justify-center rounded-full p-1 hover:shadow-lg active:scale-95 transition-transform bg-white"
          >
